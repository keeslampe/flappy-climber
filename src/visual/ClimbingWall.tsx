...
export function ClimbingWall({ world, worldWidth, groundY }: Props) {
  ...
  // Texture elements generated from a stable seed so they don't swim on scroll.
  // We use WALL_SEED × 97 + 13 to match the design bundle's rng initialisation.
  ...
  const strata: Array<{ path: string; dark: boolean }> = [];
  ...
  const cracks: Array<{ path: string; strokeWidth: number; branches: string[] }> = [];
  ...
  const pockets: Array<{ cx: number; cy: number; rx: number; ry: number }> = [];
  ...
  const pebbles: Array<{ cx: number; cy: number; rx: number; ry: number; dark: boolean }> = [];
  ...
  return (
    <g>
      ...
      <g clipPath={`url(#${clipId})`}>
        {/* Strata bands — two copies for seamless scroll */}
        {[-scrollOffset, worldWidth - scrollOffset].map((offsetX) => (
